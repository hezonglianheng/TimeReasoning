...
if __name__ == "__main__":
    ...
    # 设置查询语句
    sparql.setQuery("""
    SELECT ?item ?itemLabelZh ?itemLabelEn   ?inception ?type
    WHERE {
        VALUES ?type {wd:Q11483816}
        ?item 
        wdt:P31 ?type;         # 词条类型是 annual event
        wdt:P571 ?inception.      # 具有成立或建立时间属性
    ...
    FILTER (?inception >= "1900-01-01T00:00:00Z"^^xsd:dateTime && ?inception <= "2010-12-31T23:59:59Z"^^xsd:dateTime)
    ...
    # 将结果写入文件
    with open("wikidata_annualevents.json", "w", encoding="utf8") as f:
