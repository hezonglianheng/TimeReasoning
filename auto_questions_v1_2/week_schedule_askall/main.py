...
if __name__ == "__main__":
    curr_scene = scene.LoopScene(ts.TimeScale.Weekday, "小明是一名大学生，以下是他的每周安排", ask_mode='tag', tag=["时刻-具体值"])
    ...
    # 随机抽取事件
    all_combinations = list(combinations((learn_japanese, date, play_badminton, arrange, read_papers, meeting, movie, enjoy_fiction, guitar, jogging), 8))
    samples = random.sample(all_combinations, 10)
    res = []
    for s in samples:
        curr_scene.add_events(*s)
        # res.extend(curr_scene.run())
        res.extend(curr_scene.run_ask_all(1))
        curr_scene.reset()
